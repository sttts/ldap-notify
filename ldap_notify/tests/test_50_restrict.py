...
class TestRestrict(LocalLDAPTests):
    ...
    @patch('ldap_notify.main.run', wraps=ldap_notify.main.run)
    @patch('smtplib.SMTP')
    def test_30_restricted_users_are_case_insentive(self, mock_smtp, mock_run, mock_stderr):
        # prepare users
        for days in [-2, 0.5, 2, 5, 9, 20, 35]:
            self.addUser(expire=days, mail=True)
            
        self.addUser(expire=23, mail=True, name="myTestUser")
        self.addUser(expire=23, mail=True, name="mySeCONDTestUser")
        
        # call tool
        rc = main(['-c', os.path.dirname(__file__) + "/password.conf", '--restrict', 'myTESTUSER;CN=mysecondTESTuser,OU=users,DC=LOCALHOST'])
        self.assertEqual(mock_stderr.getvalue(), '')
        self.assertEqual(rc, 0)
        ...
        self.assertEqual(map(lambda x: x[1][1], SMTP.sendmail.mock_calls), ['myTestUser@localhost', 'mySeCONDTestUser@localhost', config.admin.to_address])
